...
const state = {
  ...
  outreachCurrentTargetsExpanded: false,
  outreachCurrentTargetsJobId: null,
  outreachExpandedJobIds: new Set(),
  tableErrors: {},
...
function renderOutreachJob(job) {
  ...
  if (!job) {
    ...
    state.outreachCurrentTargetsExpanded = false;
    state.outreachCurrentTargetsJobId = null;
    ...
    if (els.outreachCurrentTargetsToggle) {
      els.outreachCurrentTargetsToggle.disabled =
        true;
    }
  ...
  if (
    state.outreachCurrentTargetsJobId !==
    currentJobId
  ) {
    state.outreachCurrentTargetsJobId =
      currentJobId;

    state.outreachCurrentTargetsExpanded =
      false;
  }
...
function renderOutreachHistory(
  ...
  if (!rows.length) {
    ...
    state.outreachExpandedJobIds.clear();
    return;
  }
  ...
  for (
    const jobId
    of state.outreachExpandedJobIds
  ) {
    if (!visibleJobIds.has(jobId)) {
      state.outreachExpandedJobIds.delete(
        jobId
      );
    }
  }
  ...
  els.outreachHistoryBody.innerHTML =
    rows
      .map((job) => {
        ...
        const expanded =
          state.outreachExpandedJobIds.has(
            jobId
          );

        const detailHtml = expanded
          ? `
            <tr class="outreach-history-detail-row">
              <td colspan="10">
      ...
      button.addEventListener(
        ...
        () => {
          ...
          if (
            ...
            state.outreachExpandedJobIds.add(
              jobId
            );
          }
